...
def _validate(config):
    ...
    if config.get(CONF_TYPE) == "LAN867X":
        # Check if the ESP-IDF framework is used
        if not CORE.using_esp_idf:
            raise cv.Invalid("The LAN867X Ethernet PHY requires the ESP-IDF framework.")

        # Check the ESP-IDF version
        framework_version = CORE.data[KEY_CORE][KEY_FRAMEWORK_VERSION]
        required_version = cv.Version(5, 3, 1)
        if framework_version < required_version:
            raise cv.Invalid(
                f"The LAN867X Ethernet PHY is only supported on ESP-IDF {required_version} and later."
            )
    return config
