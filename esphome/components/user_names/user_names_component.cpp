...
static const char *const TAG = "user_names";
...
void DallasTemperatureSearcher::restore_sensors_count_() {
  if (this->records_count_pref_.load(&this->saved_records_num_)) {
    ESP_LOGD(TAG, "Successfully restored records count from memory - %d", this->saved_records_num_);
  } else {
