...
# WARNING: this template is stale — the chip components' __init__.py files have
# been hand-edited since it was last updated, and regenerating from it would drop
# IS_TARGET_PLATFORM = True from all three and reset ln882x's CODEOWNERS to
# @kuba2k2. Discard the __init__.py output (keep only boards.py and const.py)
# until the template is fixed; upstream did that in 3d74d1e7f0.
BASE_CODE_INIT = """
