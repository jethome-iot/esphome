...
class UpdateManifestParserInterface {
 public:
  virtual bool parse(const std::string &data, update::UpdateInfo &info, const std::string &source_url = "") = 0;
};

class DefaultUpdateManifestParser : public UpdateManifestParserInterface {
 public:
  bool parse(const std::string &data, update::UpdateInfo &info, const std::string &source_url = "") override;
};
